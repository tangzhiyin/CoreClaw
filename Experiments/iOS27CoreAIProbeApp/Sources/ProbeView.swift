...
@available(iOS 27.0, *)
@Generable(description: "A CoreClaw skill routing decision.")
private struct GuidedSkillRoute {
    @Guide(description: "Routing action. Use useSkill for requests that create, schedule, modify, or execute an action. Use answerDirectly only when CoreClaw should reply with text and perform no action.", .anyOf(["answerDirectly", "useSkill", "askClarification"]))
    var action: String
...
struct ProbeView: View {
        ...
        if #available(iOS 27.0, *) {
            do {
                ...
                let response = try await session.respond(
                    to: "Say that CoreClaw can access the iOS 27 system language model."
                )
    ...
    private func runRouterJSONTest() async {
        ...
        if #available(iOS 27.0, *) {
            do {
                let session = LanguageModelSession(
                    instructions: """
                    You are CoreClaw's routing model. Return exactly one compact JSON object and no Markdown.
                    Schema:
    ...
    You are CoreClaw's deterministic skill router. Use the provided schema.
    Policy:
    - answerDirectly means CoreClaw only replies with text and performs no action.
    - useSkill means CoreClaw invokes one of the listed Skills/tools.
