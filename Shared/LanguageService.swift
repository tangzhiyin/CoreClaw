...
// MARK: - CoreClaw Language Service
//
...
final class LanguageService {
    ...
    private static let defaultsKey = "CoreClaw.appLanguage"
...
private enum JapaneseFallbackLocalization {
    private static let exact: [String: String] = [
        ...
        "CoreClaw processes chat, image understanding, voice, and tool execution locally by default. Chat content, images, and personal data are not uploaded to CoreClaw servers.": "CoreClawは既定でチャット、画像理解、音声、ツール実行を端末内で処理します。チャット内容、画像、個人データはCoreClawサーバーへアップロードされません。",
        ...
        "CoreClaw does not use App Tracking Transparency tracking and does not use your data to track you across apps or websites.": "CoreClawはApp Tracking Transparencyによる追跡を使用せず、アプリやWebサイトをまたいでユーザーを追跡する目的でデータを使用しません。",
        "This summary explains how CoreClaw handles data locally on device and when it accesses system permissions.": "この概要では、CoreClawが端末内でデータを処理する方法と、システム権限へアクセスするタイミングを説明します。",
    ]
