...
// 通过 LiveLandAudioIO.audioInputHandler 接收 16kHz 采样 (不自建 engine, 不装 tap)。
...
class LiveLandVADService {
    ...
    var liveConfig = LiveLandVADConfig()

    // MARK: - Dependencies

    private var vadManager: VadManager?
    private var streamState: VadStreamState?
    private weak var audioIO: LiveLandAudioIO?
    ...
    func initialize() async {
        ...
        do {
            ...
            if let vadDir = LiveModelDefinition.resolve(for: LiveModelDefinition.vad) {
                ...
                print("[LiveLandVAD] Using pre-downloaded model at \(vadDir.path)")
            } else {
                // Fallback: let FluidAudio auto-download (legacy path)
                print("[LiveLandVAD] ⚠️ Pre-downloaded model not found, falling back to FluidAudio auto-download")
                manager = try await VadManager()
            }
            vadManager = manager
            streamState = await manager.makeStreamState()
            isAvailable = await manager.isAvailable
            print("[LiveLandVAD] Initialized, available: \(isAvailable)")
        } catch {
            print("[LiveLandVAD] ❌ Init failed: \(error)")
            isAvailable = false
        }
    }
    ...
    func startListening(audioIO: LiveLandAudioIO) async {
        guard isAvailable, let vadManager else {
            print("[LiveLandVAD] Not available")
            return
        }
    ...
    // MARK: - Audio Input (called from LiveLandAudioIO tap, on audio thread)
    ...
    /// See testCallbackOrdering in LiveLandVADServiceTests.
    internal func processChunk(_ chunk: [Float]) async {
        ...
        do {
            ...
            print("[LiveLandVAD] ❌ Process error: \(error)")
        }
    }
    ...
    internal func handleStreamingResult(_ result: VadStreamResult, chunk: [Float]) {
        ...
        if let event = result.event {
            ...
            case .speechStart:
                ...
                print("[LiveLandVAD] 🎤 Speech start")
                onSpeechStart?()
                onSpeechChunk?(chunk)
            ...
            case .speechEnd:
                ...
                print("[LiveLandVAD] 🔇 Speech end (\(String(format: "%.1f", duration))s)")
