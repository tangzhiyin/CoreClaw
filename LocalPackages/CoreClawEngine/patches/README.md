...
本目录是 CoreClaw 对 LiteRT-LM (Google's iOS LLM 推理库) 的本地修改归档,
用于在 `LocalPackages/CoreClawEngine/Frameworks/LiteRTLM.xcframework` 需要
重建时, 不丢失这两处关键 patch。
...
# 方法 A — 直接用 .patch
cd /tmp/LiteRT-LM
git apply <CoreClaw>/LocalPackages/CoreClawEngine/patches/litert-lm.patch
```

```bash
# 方法 B — 直接覆盖完整文件 (上游若与归档时大幅 drift, A 会失败, 用 B)
cp <CoreClaw>/LocalPackages/CoreClawEngine/patches/sampler_factory.cc \
   /tmp/LiteRT-LM/runtime/components/sampler_factory.cc
cp <CoreClaw>/LocalPackages/CoreClawEngine/patches/c-BUILD \
...
```bash
cp <CoreClaw>/LocalPackages/CoreClawEngine/patches/package-xcframework.sh /tmp/v3-dylibs/
chmod +x /tmp/v3-dylibs/package-xcframework.sh
/tmp/v3-dylibs/package-xcframework.sh
```

脚本会**直接覆盖** `<CoreClaw>/LocalPackages/CoreClawEngine/Frameworks/LiteRTLM.xcframework/`。

### 5. 验证

```bash
# device slice 应该有 4 个 dylib
ls -la <CoreClaw>/LocalPackages/CoreClawEngine/Frameworks/LiteRTLM.xcframework/ios-arm64/CLiteRTLM.framework/

# 在 iPhone 上运行 CoreClaw, 控制台应出现:
