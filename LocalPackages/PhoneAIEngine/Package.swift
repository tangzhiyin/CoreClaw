...
//
// 三个 target, 都通过 library product `PhoneAIEngine` 暴露:
//
//   PhoneAIEngine    Swift  Gemma 4 / LiteRT-LM (.litertlm)
//                             deps: CLiteRTLM
...
//                + LocalPackages/PhoneAIEngine/patches/ 上游补丁归档。
...
let package = Package(
    ...
    products: [
        .library(name: "PhoneAIEngine", targets: ["PhoneAIEngine", "MTMDEngine"]),
    ],
    targets: [
        // ───────────────────────────────────────────────────────────
