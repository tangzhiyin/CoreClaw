...
本目录是 PhoneAI 对 LiteRT-LM (Google's iOS LLM 推理库) 的本地修改归档,
用于在 `LocalPackages/PhoneAIEngine/Frameworks/LiteRTLM.xcframework` 需要
重建时, 不丢失这两处关键 patch。
...
# 方法 A — 直接用 .patch
cd /tmp/LiteRT-LM
git apply <PhoneAI>/LocalPackages/PhoneAIEngine/patches/litert-lm.patch
```

```bash
# 方法 B — 直接覆盖完整文件 (上游若与归档时大幅 drift, A 会失败, 用 B)
cp <PhoneAI>/LocalPackages/PhoneAIEngine/patches/sampler_factory.cc \
   /tmp/LiteRT-LM/runtime/components/sampler_factory.cc
cp <PhoneAI>/LocalPackages/PhoneAIEngine/patches/c-BUILD \
...
```bash
cp <PhoneAI>/LocalPackages/PhoneAIEngine/patches/package-xcframework.sh /tmp/v3-dylibs/
chmod +x /tmp/v3-dylibs/package-xcframework.sh
/tmp/v3-dylibs/package-xcframework.sh
```

脚本会**直接覆盖** `<PhoneAI>/LocalPackages/PhoneAIEngine/Frameworks/LiteRTLM.xcframework/`。

### 5. 验证

```bash
# device slice 应该有 4 个 dylib
ls -la <PhoneAI>/LocalPackages/PhoneAIEngine/Frameworks/LiteRTLM.xcframework/ios-arm64/CLiteRTLM.framework/

# 在 iPhone 上运行 PhoneAI, 控制台应出现:
