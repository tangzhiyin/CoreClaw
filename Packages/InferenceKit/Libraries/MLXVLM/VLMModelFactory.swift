...
/// from configuration.
///
/// NOTE: Slimmed to Gemma multimodal family (paligemma + gemma3) only for CoreClaw.
/// Upstream mlx-swift-lm registers many VLM types (Qwen2-VL, Qwen3-VL, Idefics3, SmolVLM,
/// FastVLM, Pixtral, Mistral3, LFM2-VL, GlmOcr, ...). CoreClaw uses Gemma 4 multimodal
...
public enum VLMProcessorTypeRegistry {
    ...
    /// Slimmed to match `VLMTypeRegistry` — CoreClaw registers `Gemma4Processor` at runtime.
...
///
/// NOTE: Slimmed to Gemma multimodal family only for CoreClaw. See `VLMTypeRegistry` above.
