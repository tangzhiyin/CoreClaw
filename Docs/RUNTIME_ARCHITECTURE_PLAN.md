...
| Plan 章节 | 状态 | 备注 |
|---|---|---|
| §3.2 LiteRTBootstrap | ✅ 完整 | `LLM/Core/LiteRTBootstrap.swift` + CoreClawApp.init 第一行;Mac CLI 路径 `#if canImport(CoreClawEngine)` 守护 |
...
做的事：
  ...
  - 从 CoreClawEngine.swift 的 _preloadGpuAcceleratorOnce 挪到 Bootstrap
  - CoreClawApp.init() 第一行调 LiteRTBootstrap.bootstrap()
...
> 这些规则是架构约束，不是打包阶段临时修。CoreClaw 已经踩过裸 dylib 拒包、
...
| F6 | 不要手动塞 `libswift*.dylib` 到 `Payload/CoreClaw.app/Frameworks/`。Swift runtime / SwiftSupport 交给 Xcode archive/export 处理 | 签名冲突 / 拒包 |
...
| CoreClawEngine (C wrapper) | 保持 | `destroySynchronously()` 已够用 |
