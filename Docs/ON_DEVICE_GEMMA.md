...
PhoneAI is a private local iPhone Agent powered by Gemma. It runs inference and Skill calls on-device, without requiring a cloud model API.
...
Gemma is a good fit for a local mobile agent because it can handle short and medium context tasks while staying small enough for iPhone-class devices. In PhoneAI, Gemma is used for:
...
That is why PhoneAI currently treats short and medium context workflows as the primary product target. The goal is not to replace cloud-scale long-context chat. The goal is to make local personal tasks reliable:
...
PhoneAI does not expose every tool blindly to the model. It uses:
