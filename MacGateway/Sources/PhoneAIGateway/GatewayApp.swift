...
private enum GatewayAppIcon {
    ...
    private static func iconURL() -> URL? {
        ...
        let relativePaths = [
            "PhoneAIGateway_PhoneAIGateway.bundle/Contents/Resources/AppIcon.icns",
            "PhoneAIGateway_PhoneAIGateway.bundle/AppIcon.icns"
        ]
...
enum GatewayKeychain {
    private static let service = "ai.phoneai.gateway.providers"
...
final class GatewayModel {
    ...
    func start() {
        ...
        do {
            ...
            recordEvent(gatewayStatus)
            NSLog("[PhoneAIGateway] start failed: \(error.localizedDescription)")
        }
        refreshPaired()
    ...
    private static func providerExtraHeaders(for kind: ProviderKind) -> [String: String] {
        ...
        case .openRouter:
            return [
                "HTTP-Referer": "https://phoneai.local",
                "X-Title": "PhoneAI Gateway",
            ]
        default:
            return [:]
        }
    ...
    private static let configsKey = "PhoneAI.gateway.providers"
