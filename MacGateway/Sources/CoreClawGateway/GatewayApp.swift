...
// MARK: - CoreClaw Gateway (C · macOS 客户端)
//
...
@main
struct CoreClawGatewayApp: App {
    @NSApplicationDelegateAdaptor(AppDelegate.self) private var delegate
    @State private var model = GatewayModel()

    var body: some Scene {
        WindowGroup("CoreClaw Gateway", id: "main") {
            GatewayDashboardView(model: model)
        }
        .defaultSize(width: 1280, height: 854)

        MenuBarExtra("CoreClaw Gateway", systemImage: "antenna.radiowaves.left.and.right") {
...
private enum GatewayAppIcon {
    ...
    private static func iconURL() -> URL? {
        ...
        let relativePaths = [
            "CoreClawGateway_CoreClawGateway.bundle/Contents/Resources/AppIcon.icns",
            "CoreClawGateway_CoreClawGateway.bundle/AppIcon.icns"
        ]
...
final class GatewayModel {
    ...
    let macID: String = {
        let key = "CoreClaw.gateway.macID"
    ...
    func start() {
        ...
        do {
            ...
            recordEvent(gatewayStatus)
            NSLog("[CoreClawGateway] start failed: \(error.localizedDescription)")
        }
        refreshPaired()
    ...
    private static let configsKey = "CoreClaw.gateway.providers"
